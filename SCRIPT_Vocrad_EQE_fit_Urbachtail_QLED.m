...
close all
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% NOTE: !!!!(input EQE data should be a two-column textfile (.txt) (x:energy [eV],y:EQE [absolut values]))!!!!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% !!!! ENTER FILE NAME and your solar cell parameters:
...
manually='no';                          % choose 'yes' or 'no' if you want to chose the transition point manually 'yes' if a manual correction is necessary or automatically 'no'
...
% Import data from text file. The name must be manually entered
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
EQEdata=load(enter_filename); %load EQE-measurement
%EQEdata=load('18R_11421_1_02_GM_QE_20180810_125235_E_EQE.dat'); %load EQE-measurement
E_EQE_original=EQEdata(:,1);                 % [eV] energie (x-axis, of this data set)
...
% Load data bank containing non-radiave voltage loss for other pioneering device work
    if strcmpi(comparsion_literature, 'yes')
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        LitSurvey = xlsread('LiteratureSurvey.xlsx', 'A3:L23'); % load data bank
        ...
        set(gcf,'units','normalized','position',[0.1 0.15 0.8 0.7])
        Eg = 1.6; % Need to populate this value with Eg determined from tauc EQE/2
        %Rad_Voc = Voc_rad;
        bar(Eg);
        hold on;
        bar(Voc_rad);
...
%unimportant at all:
clear q c h h_eVs h_Js vor VT Jsc_rad Jo_rad FF_rad eff_rad
